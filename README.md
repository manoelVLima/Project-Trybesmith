...
Trata-se de construir uma API com NodeJs e Typescript, usar métodos http de CRUD, enviando requisições ao banco de dados MySQL.Usando a arquitetura de camadas MSC para organizar as responsabilidades da aplicação e manter o código mais limpo e legível possível. Foi utilizado também as bibliotecas jsonwebtoken e jwt para geração/validação de token e validações do corpo das requisições respectivamente.
